(function() {
  ...
  local_methods = {
    ...
    column_types: function(me, table_name) {
      var R, ref1, row;
      R = {};
      ref1 = me.$.query(me.$.interpolate("pragma table_info( $table_name );", {table_name}));
    ...
    //---------------------------------------------------------------------------------------------------------
    _dependencies_of: function(me, table_name, schema = 'main') {
      return me.$.query(`pragma foreign_key_list( ${schema}.${me.$.as_sql(table_name)} )`);
    },
    //---------------------------------------------------------------------------------------------------------
    dependencies_of: function(me, table_name, schema = 'main') {
      var row;
      validate.ic_schema(schema);
      return (function() {
        var ref1, results;
        ref1 = me.$._dependencies_of(table_name);
        results = [];
        for (row of ref1) {
