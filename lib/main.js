...
order by
  ...
  p.cid;`));
      ...
      attach(cfg) {
        var schema, schema_x;
        ({schema, schema_x} = this._schema_from_cfg(cfg));
        this._debug('^33443^', cfg);
        this._debug('^33443^', this._schema_from_cfg(cfg));
        validate.ic_schema(schema);
        validate.ic_path(cfg.path);
        return this.execute(`attach ${this.as_sql(cfg.path)} as ${schema_x};`);
      }
      ...
      //-----------------------------------------------------------------------------------------------------------
      copy_schema(cfg) {
        var d, fk_state, from_schema, from_schema_x, i, inserts, j, len, len1, name_x, objects, ref1, ref2, ref3, schemas, sql, to_schema, to_schema_x;
