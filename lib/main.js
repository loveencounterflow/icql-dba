(function() {
  'use strict';
  var CND, FS, HOLLERITH, L, LFT, Multimix, badge, debug, declare, echo, help, info, isa, new_bsqlt_connection, rpr, size_of, type_of, urge, validate, validate_optional, warn, whisper,
    indexOf = [].indexOf;
  ...
  new_bsqlt_connection = require('better-sqlite3');
  ...
  this.Dba = (function() {
    class Dba extends Multimix {
      //---------------------------------------------------------------------------------------------------------
      constructor(cfg) {
        var bsqlt3_cfg, path, schema;
        super();
        this._statements = {};
        this._schemas = {};
        cfg = {...this.constructor._defaults, ...cfg};
        this._dbg = {
          debug: cfg.debug,
          echo: cfg.echo
        };
        if (cfg.sqlt != null) {
          throw new Error("^icql-dba.open@445^ argument `sqlt` not supported (yet)");
        }
        path = L.pick(cfg, 'path', this.constructor._defaults.path, 'ic_path');
        schema = L.pick(cfg, 'schema', this.constructor._defaults.schema, 'ic_schema');
        bsqlt3_cfg = {
        ...
        if (schema === 'main') {
          this.sqlt = new_bsqlt_connection(path, bsqlt3_cfg);
          this._schemas[schema] = {path};
        } else {
          this.sqlt = new_bsqlt_connection('', bsqlt3_cfg);
      ...
      open(cfg) {
        ...
        if (this.has({schema})) {
          throw new Error(`^icql-dba.open@445^ schema ${rpr(schema)} already exists`);
        }
