(function() {
  'use strict';
  var CND, Checks_mixin, E, FS, Functions_mixin, Import_export_mixin, L, PATH, SQL, Stdlib_mixin, TMP, badge, debug, echo, freeze, guy, help, info, lets, misfit, new_bsqlt3_connection, ref1, rpr, types, urge, warn, whisper,
    indexOf = [].indexOf,
    ...
    class Dba extends Stdlib_mixin(Checks_mixin(Functions_mixin(Import_export_mixin()))) {
      ...
      constructor(cfg) {
        ...
        if (this.cfg.schema != null) {
          ...
          throw new E.Dba_cfg_error('^dba@300^', "property `schema` not supported");
        }
        ...
        this._catalog = freeze({});
        this./* NOTE: will hold data on user-defined functions, virtual tables */_state.initialized = true/* TAINT remove */
        this._initialize_prng();
        //.......................................................................................................
        ({ram, name, path} = this.cfg);
        if (ram) {
          ({url, name} = dba._get_connection_url(name != null ? name : null));
          debug('^3443^', name, url);
          if (path != null) {
        ...
        this.sqlt = new_bsqlt3_connection(path, this._bsqlt3_cfg);
        this.sqlt2 = new_bsqlt3_connection(path, this._bsqlt3_cfg);
        this.initialize_sqlt(this.sqlt);
        //.......................................................................................................
        return void 0;
      }
      ...
      save(cfg) {
        ...
        path = (ref2 = (ref3 = this._schemas[schema]) != null ? ref3.path : void 0) != null ? ref2 : null;
        if (path == null) {
  ...
  where name like ?;`;
      ...
      _max_temp_schema_number() {
        var matcher, ref2, sql;
        matcher = this.cfg._temp_prefix + '%';
        sql = `select
    max( cast( substring( name, ? ) as integer ) ) as n
  from pragma_database_list
  where name like ?;`;
        return (ref2 = this.first_value(this.query(sql, [this.cfg._temp_prefix.length + 1, matcher]))) != null ? ref2 : 0;
      }
  ...
  order by seq, type ${ordering_x}, name;`);
      ...
      //---------------------------------------------------------------------------------------------------------
      _walk_all_objects() {
        var d, parts, ref2, row, schema, schema_i, schema_l, schemas, sql;
        schemas = {};
        parts = [];
        ref2 = this.query(SQL`select seq, name, file as path from pragma_database_list order by seq;`);
        //.......................................................................................................
        /* TAINT use API */
        for (row of ref2) {
...
from ${schema_i}.sqlite_schema as d1`);
      ...
      list_schema_names() {
        var d, i, len, ref2, results;
        ref2 = this.list_schemas();
        results = [];
      ...
      has(cfg) {
        ...
        return ref2 = cfg.schema, indexOf.call(this.list_schema_names(), ref2) >= 0;
      }
      ...
      get_schemas() {
        var R, ref2, row;
        R = {};
        ref2 = this.query("select * from pragma_database_list order by seq;");
        for (row of ref2) {
          R[row.name] = row.file;
        }
        return R;
...
limit 1;`, {name}));
      ...
      //---------------------------------------------------------------------------------------------------------
      fields_of(cfg) {
        var R, d, name, ref2, schema, schema_i, type;
        this.types.validate.dba_fields_of_cfg((cfg = {...this.types.defaults.dba_fields_of_cfg, ...cfg}));
        ({name, schema} = cfg);
        schema_i = this.sql.I(schema);
        R = {};
        ref2 = this.query(SQL`select * from ${schema_i}.pragma_table_info( $name );`, {name});
        for (d of ref2) {
      ...
      field_names_of(cfg) {
        ...
        return (function() {
          var ref2, results;
          ref2 = this.query(SQL`select name from ${schema_i}.pragma_table_info( $name );`, {name});
          results = [];
          for (d of ref2) {
            results.push(d.name);
          }
      ...
      dependencies_of(table, schema = 'main') {
        ...
        return (function() {
          var ref2, results;
          ref2 = this._dependencies_of(table);
          results = [];
          for (row of ref2) {
      ...
      clear(cfg) {
        var R, fk_state, i, len, name, ref2, schema, schema_i, statement, type;
        ...
        ref2 = this.list(this.walk_objects({
          schema,
          _ordering: 'drop'
        }));
        for (i = 0, len = ref2.length; i < len; i++) {
...
/* TAINT use API call */        return null;
      ...
      _copy_or_move_schema(cfg, detach_schema = false) {
        var d, detach_from_schema, fk_state, from_schema, from_schema_objects, from_schema_x, i, inserts, j, known_schemas, len, len1, name_x, ref2, ref3, sql, to_schema, to_schema_objects, to_schema_x;
...
//.......................................................................................................
      ...
      _initialize_prng() {
        var clasz, delta, ref2, ref3, seed;
        clasz = this.constructor;
        if (clasz._rnd_int_cfg != null) {
          seed = (ref2 = clasz._rnd_int_cfg.seed) != null ? ref2 : 12.34;
          delta = (ref3 = clasz._rnd_int_cfg.delta) != null ? ref3 : 1;
      ...
      _get_connection_url(name = null) {
        ...
        if (name == null) {
          name = `_icql_${this._rnd_int(1_000_000_000, 9_999_999_999)}`;
        }
