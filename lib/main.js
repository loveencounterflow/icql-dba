(function() {
  'use strict';
  var CND, FS, HOLLERITH, L, LFT, Multimix, badge, bsqlt3, debug, declare, echo, help, info, isa, rpr, size_of, type_of, urge, validate, validate_optional, warn, whisper,
    indexOf = [].indexOf;
    ...
    class Dba extends Multimix {
      ...
      constructor(cfg) {
        ...
        if (this.cfg.sqlt != null) {
          this.sqlt = this.cfg.sqlt;
        } else if (schema === 'main') {
          this.sqlt = bsqlt3(path, bsqlt3_cfg);
        } else {
          this.sqlt = bsqlt3('', bsqlt3_cfg);
...
from ${schema_i}.sqlite_schema as d1`);
      ...
      detach(cfg) {
        ...
        schema = L.pick(cfg, 'schema', null, 'ic_schema');
        schema_i = this.as_identifier(schema);
        return this.execute(`detach ${schema_i};`);
      }
