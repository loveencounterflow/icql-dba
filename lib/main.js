(function() {
  'use strict';
  var CND, E, FS, HOLLERITH, Import_export_mixin, L, PATH, Sql_mixin, TMP, badge, debug, declare, echo, freeze, help, info, isa, lets, misfit, new_bsqlt3_connection, rpr, size_of, sql, type_of, types, urge, validate, validate_optional, warn, whisper,
    indexOf = [].indexOf;
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.Dba = class Dba extends Import_export_mixin(Sql_mixin()) {
    //---------------------------------------------------------------------------------------------------------
    constructor(cfg) {
      var bsqlt3_cfg;
      super();
  ...
  order by seq, type ${ordering_x}, name;`);
    ...
    //---------------------------------------------------------------------------------------------------------
    _walk_all_objects() {
      var d, parts, ref1, row, schema, schema_i, schema_s, schemas;
      schemas = {};
      parts = [];
...
from ${schema_i}.sqlite_schema as d1`);
    ...
    _copy_or_move_schema(cfg, detach_schema = false) {
      var d, detach_from_schema, fk_state, from_schema, from_schema_objects, from_schema_x, i, inserts, j, known_schemas, len, len1, name_x, ref1, ref2, to_schema, to_schema_objects, to_schema_x;
