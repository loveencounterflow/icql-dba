(function() {
  ...
  local_methods = {
    ...
    list_objects: function(me, schema = 'main') {
      ...
      return this.query(`select * from ${this.as_identifier(schema)}.sqlite_master order by type desc, name;`);
    },
    ...
    attach: function(me, path, schema) {
      validate.ic_schema(schema);
      return this.execute(`attach '${path}' as [${this.as_identifier(schema)}];`);
    },
    //-----------------------------------------------------------------------------------------------------------
