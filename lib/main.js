(function() {
  'use strict';
  var CND, E, FS, HOLLERITH, Import_export_mixin, L, PATH, SQL, TMP, badge, debug, declare, echo, freeze, guy, help, info, isa, lets, misfit, new_bsqlt3_connection, rpr, size_of, type_of, types, urge, validate, validate_optional, warn, whisper,
    indexOf = [].indexOf;
  ...
  where name like ?;`;
    ...
    * first_values(iterator) {
    ...
    //---------------------------------------------------------------------------------------------------------
    do_unsafe(f) {
      this.sqlt.unsafeMode(true);
      try {
        return f();
      } finally {
        this.sqlt.unsafeMode(false);
      }
    }

    async do_unsafe_async(f) {
      this.sqlt.unsafeMode(true);
      try {
        return (await f());
      } finally {
    ...
    create_function(cfg) {
      ...
      validate.dba_create_function_cfg((cfg = {...this.types.defaults.dba_create_function_cfg, ...cfg}));
    ...
    create_aggregate_function(cfg) {
      ...
      validate.dba_create_aggregate_function_cfg((cfg = {...this.types.defaults.dba_create_aggregate_function_cfg, ...cfg}));
    ...
    create_window_function(cfg) {
      ...
      validate.dba_create_window_function_cfg((cfg = {...this.types.defaults.dba_create_window_function_cfg, ...cfg}));
    ...
    create_table_function(cfg) {
      ...
      validate.dba_create_table_function_cfg((cfg = {...this.types.defaults.dba_create_table_function_cfg, ...cfg}));
    ...
    create_virtual_table(cfg) {
      var create, name;
      validate.dba_create_virtual_table_cfg((cfg = {...this.types.defaults.dba_create_virtual_table_cfg, ...cfg}));
      ({name, create} = cfg);
...
from ${schema_i}.sqlite_schema as d1`);
    ...
    type_of(cfg) {
      var name, row, schema;
      validate.dba_type_of_cfg((cfg = {...this.types.defaults.dba_type_of_cfg, ...cfg}));
      ({name, schema} = cfg);
...
limit 1;`, {name}));
    ...
    fields_of(cfg) {
      ...
      validate.dba_fields_of_cfg((cfg = {...this.types.defaults.dba_fields_of_cfg, ...cfg}));
      ({name, schema} = cfg);
      schema_i = this.sql.I(schema);
      R = {};
    ...
    field_names_of(cfg) {
      ...
      // try
      validate.dba_field_names_of_cfg((cfg = {...this.types.defaults.dba_field_names_of_cfg, ...cfg}));
      ({name, schema} = cfg);
      schema_i = this.sql.I(schema);
      return (function() {
        var ref1, results;
    ...
    dump_relation(cfg) {
      ...
      validate.dba_dump_relation_cfg((cfg = {...this.types.defaults.dba_dump_relation_cfg, ...cfg}));
    ...
    clear(cfg) {
      ...
      validate.dba_clear_cfg((cfg = {...this.types.defaults.dba_clear_cfg, ...cfg}));
      ({schema} = cfg);
      schema_i = this.sql.I(schema);
      R = 0;
    ...
    _open_file_db_in_ram(cfg) {
      ...
      schema_main_allowed = !this._initialized;
