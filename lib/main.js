...
(function() {
  'use strict';
  var CND, FS, IC, assign, badge, debug, declare, echo, help, info, inspect, isa, jr, local_methods, rpr, size_of, type_of, types, urge, validate, warn, whisper, xrpr;
  ...
  local_methods = {
    ...
    //---------------------------------------------------------------------------------------------------------
    run_or_query: function(me, sql, Q, discard) {
      ...
      try {
        ...
        error = error1;
        if (error.name === 'TypeError' && error.message.startsWith('This statement returns data.')) {
          if (discard) {
            if (Q != null) {
    ...
    //---------------------------------------------------------------------------------------------------------
    query_or_run: function(me, sql, Q, discard) {
      ...
      try {
        ...
        error = error1;
        if (error.name === 'TypeError' && error.message.startsWith('This statement does not return data.')) {
          return statement.run(Q);
        }
        throw error;
      }
...
//.........................................................................................................
  ...
  this._method_from_ic_entry = function(me, ic_entry) {
    ...
    switch (ic_entry.type) {
      case 'procedure':
        endpoint = me.$.run_or_query;
        break;
      case 'query':
        endpoint = me.$.query_or_run;
    }
    //.........................................................................................................
    return (Q) => {
      var R, descriptor, i, idx, last_idx, len, part, ref;
      descriptor = this._descriptor_from_arguments(me, ic_entry, Q);
      last_idx = descriptor.parts.length - 1;
      ref = descriptor.parts;
      for (idx = i = 0, len = ref.length; i < len; idx = ++i) {
        part = ref[idx];
        R = endpoint(part, Q, idx !== last_idx);
      }
      return R;
    };
  };
