(function() {
  'use strict';
  var CND, E, FS, Functions_mixin, HOLLERITH, Import_export_mixin, L, PATH, SQL, TMP, badge, debug, echo, freeze, guy, help, info, lets, misfit, new_bsqlt3_connection, rpr, types, urge, warn, whisper,
    indexOf = [].indexOf;
  ...
  where name like ?;`;
    ...
    * first_values(iterator) {
    ...
    //---------------------------------------------------------------------------------------------------------
    _get_unsafe_mode_state() {
      return this._in_unsafe_mode;
    }

    //---------------------------------------------------------------------------------------------------------
    _set_unsafe_mode_state(onoff) {
      this.types.validate.boolean(onoff);
      this._in_unsafe_mode = onoff;
      return null;
    }
