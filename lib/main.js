(function() {
  ...
  where name like ?;`;
      ...
      _import_sql(cfg) {
        switch (cfg.method) {
          case 'single':
            this._import_sql_single(cfg);
            break;
          case 'batch':
            this._import_sql_batch(cfg);
        }
        return null;
      }

      //---------------------------------------------------------------------------------------------------------
      static _import_sql_single(cfg) {
        db.exec(FS.readFileSync(cfg.path, {
          encoding: 'utf-8'
        }));
        return null;
      }
      ...
      //---------------------------------------------------------------------------------------------------------
      static * _walk_statements_from_path(sql_path) {
      ...
      //---------------------------------------------------------------------------------------------------------
      static * _walk_batches(iterator, batch_size = 1) {
...
from ${schema_i}.sqlite_schema as d1`);
      ...
      _attach(cfg) {
        ...
        try {
          //.......................................................................................................
          this.run("attach ? as ?;", [cfg.path, cfg.schema]);
        } catch (error1) {
          error = error1;
